...
<!-- Hero Section for "Lista de Mascotas" -->
<section class="hero text-white text-center py-5" style="background: linear-gradient(135deg, #343a40, #17a2b8);">
    <div class="container">
        <h1 class="display-4 fw-bold">Lista de Mascotas</h1>
        <p class="lead mt-3">Administra y cuida los detalles de tus mascotas favoritas con facilidad.</p>
    </div>
</section>
                ...
                <div class="modal-body">
                    <form id="formMascota" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" step="0.01" class="form-control" id="peso" name="peso" required min="0">
                        </div>
                        <div class="mb-3">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="telefono_duenio" name="telefono_duenio" required pattern="^\d{10}$">
                            <small class="form-text text-muted">El teléfono debe contener exactamente 10 dígitos.</small>
                        </div>
                        <div class="mb-3">
    ...
    <table class="table mt-3">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    <img src="{{ $mascota->imagen_url }}" alt="Imagen de {{ $mascota->nombre }}" style="width: 100px; height: 100px; object-fit: cover;">
                </td>
                <td>
                    <button class="btn btn-warning btn-sm" onclick="editarMascota('{{ $mascota->id }}', '{{ $mascota->nombre }}', {{ $mascota->edad }}, '{{ $mascota->raza }}', {{ $mascota->peso }}, '{{ $mascota->nombre_duenio }}', '{{ $mascota->telefono_duenio }}', '{{ $mascota->imagen_url }}')">
                        <i class="bi bi-pencil-fill"></i>
                    </button>
                    <form action="{{ route('mascotas.destroy', $mascota->id) }}" method="POST" style="display: inline-block;" onsubmit="return confirmarEliminacion()">
                        @csrf
                        @method('DELETE')
...
<script>
    ...
    function validarFormulario() {
        ...
        if (isNaN(peso) || peso <= 0) {
            alert('Por favor, introduce un peso válido (mayor a 0).');
            return false;
        }

        const telefonoRegex = /^\d{10}$/;
        if (!telefonoRegex.test(telefono)) {
            alert('Por favor, introduce un teléfono válido (10 dígitos).');
            return false;
        }
